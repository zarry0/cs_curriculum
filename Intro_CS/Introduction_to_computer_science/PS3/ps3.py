...
WORDLIST_FILENAME = "/Users/zarry/Documents/CS/Intro_CS/Introduction_to_computer_science/Lecture_6/PS3/words.txt"
...
def get_word_score(word, n):
    ...
    letter_scores = 0
    for letter in word:
        letter_scores += SCRABBLE_LETTER_VALUES[letter]

    score = letter_scores * max((7 * len(word) - 3 * (n - len(word) )), 1)
    return score
...
def update_hand(hand, word):
    ...
    for letter in word:
        if letter in hand:
            updated_hand[letter] = max(updated_hand[letter] - 1, 0)
    return updated_hand
...
def is_valid_word(word, hand, word_list):
    ...
    frec = get_frequency_dict(lowercase_word)
    if lowercase_word in word_list:
        for letter in lowercase_word:
            if not (letter in hand):
                return False
            elif not (frec[letter] <= hand[letter]):
                return False
        return True
    elif '*' in lowercase_word:
        possible_words = [lowercase_word.replace('*',vowel) for vowel in VOWELS]
...
def substitute_hand(hand, letter):
    ...
    def pick_random_letter():
        new = hand_keys[0]
        while new in hand_keys:
            if letter in VOWELS:
                new = random.choice(VOWELS)
            elif letter in CONSONANTS:
                new = random.choice(CONSONANTS)
        return new
    ...
    if not (letter in hand):
        ...
        for key in hand_keys:
            if key == letter:
                new_hand[pick_random_letter()] = hand[key]
            else:
...
def play_game(word_list):
    ...
    for _ in range(number_of_hands):
        ...
        if substitution or replay:
            print('Current hand:', end=' ')
            display_hand(current_hand)

        if substitution:
            change_letter = input('Would you like to substitute a letter? ').lower()
