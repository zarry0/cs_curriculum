...
/*
 ...
 +----------------------------+                                                                       | |
   ...
   | Circle                     |   | Square                     |  | Combo                      |    | |
   +----------------------------+   +----------------------------+  +----------------------------+    | |
 +-| CartPt center              | +-| CartPt nw                  |  | IShape shape1              |+---+ |
 | | int radius                 | | | int size                   |  | IShape shape2              |+-----+
 ...
 | | boolean biggerThan(IShape) | | | boolean biggerThan(IShape) |  | boolean contains(CartPt)   |
...
class Combo implements IShape {
    ...
    // produces the minimum of the distances of its components to the origin
    public double distToOrigin() {
        double distance1 = this.shape1.distToOrigin();
        double distance2 = this.shape2.distToOrigin();
        return Math.min(distance1, distance2);
    }

    // produces the total area of the combo
    public double area() {
        return this.shape1.area() + this.shape2.area();
    }
    

    // produces a new shape that is this increased in size by inc units
    public IShape grow(int inc) {
        return new Combo(this.shape1.grow(inc), this.shape2.grow(inc));
    }

    // produces true if either of its components contain the given point
    public boolean contains(CartPt point) {
        return this.shape1.contains(point) || this.shape2.contains(point);
    }

    // unimplemented method
    public boolean biggerThan(IShape that) { return false; }
