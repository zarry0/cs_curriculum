...
;#; ;;Tail recursive version of anc-names
(define (anc-names-tr at)
  (local [;; (listof String) * (listof AT) -> (listof String)
          (define (aux ans todo)
            (cond [(empty? todo) ans]
                  [else (aux (cons (person-name (first todo)) ans)
                             (append (rest todo)
                                     (build-list (person-mom (first todo))
                                                 (person-dad (first todo)))))]))
          ;; AT * AT -> (listof AT)
          ;; produces a list of only the non-empty inputs
          (define (build-list at1 at2)
            (cond [(empty? at1) (list-wrap at2)]
                  [else (cons at1 (list-wrap at2))]))
          ;; AT -> (listof AT)
          ;; produces empty if the given input is empty
          ;;          (list at) otherwise
          (define (list-wrap at)
            (cond [(empty? at) empty]
                  [else (cons at empty)]))]
    (reverse (aux empty (list-wrap at)))))
