...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname sodoku_alt) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
;; produces 9 boards, with blank filled with Natural[1, 9] 
...
(define (fill-with-1-9 p bd)
  (local [(define (build-one n) (fill-square bd p (+ 1 n)))]
    (build-list 9 build-one)))

;; (listof Board) -> (listof Board)
;; produces a list of only valid boards
(check-expect (keep-only-valid (list (cons 1 (cons 2 (rest (rest BD3)))))) empty)
...
;; valid means no unit contains a duplicate value
...
(define (valid-board? bd)
  (local [;; (listof Unit) -> Boolean
          ;; produces true if all the uits are valid
          (define (valid-units? lou)
            (andmap valid-unit? lou))

          ;; Unit -> Boolean
          ;; produces true if the unit is valid
          (define (valid-unit? u)
            (no-duplicates? (keep-only-values (read-unit u))))

          ;; Unit -> (listof Val | false)
          ;; produces a list of values corresponding to every pos in the unit
          (define (read-unit u)
            (map read-pos u))

          ;; Pos -> Val | false
          ;; produces the actual value at position p on board bd
          (define (read-pos p)
            (read-square bd p))

          ;; (listof Val | false) -> (listof Val)
          ;; produces a list with only values
          (define (keep-only-values lovf)
            (filter number? lovf))

          ;; (listof Val) -> Boolean
          ;; produces true if the list has no repeated elements
          (define (no-duplicates? lov)
            (cond [(empty? lov) true]
                  [else
                   (and (not (member (first lov) (rest lov)))
...
#;
(define (valid-board? bd)
  (local [;; (listof Unit) -> (listof (listof Val | false))
          ;; produces list of the actual values in bd at every position in each unit
          (define (get-unit-vals lou)
            (cond [(empty? lou) empty]
                  [else
                   (cons (map (lambda (p) (read-square bd p)) (first lou))
                         (get-unit-vals (rest lou)))]))

          ;; (listof (listof Val | false)) -> (listof (listof Val))
          ;; produces a list of lists of only values
          (define (keep-only-values lox)
            (cond [(empty? lox) empty]
                  [else
                   (cons (filter number? (first lox))
                         (keep-only-values (rest lox)))]))

          ;; (listof (listof Val)) -> Boolean
          ;; produces true if every sub-list doesn't have any repeated elements
          (define (validate-units? lox)
            (andmap no-duplicates? lox))
          ...
          (define (no-duplicates? lov)
            (cond [(empty? lov) true]
                  [else
                   (and (not (in (first lov) (rest lov)))
                        (no-duplicates? (rest lov)))]))

          ;; Val (listof Val) -> Boolean
          ;; produces true if v is in lov
          (define (in v lov)
    ...
    (validate-units? (keep-only-values (get-unit-vals UNITS)))))
