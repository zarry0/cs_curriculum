  ...
  (cond [(false? br) false]
        [else
         (if (string=? t (bracket-team-lost br))
             (bracket-team-won br)
             (local [(define check-winner-br (ouster t (bracket-br-won br)))]
               (if (not (false? check-winner-br))
                   check-winner-br
                   (ouster t (bracket-br-lost br)))))]))

;; JUSTIFICATION: When doing the backtracking search, it's not necessary nor efficient to go through the winner branch twice.
