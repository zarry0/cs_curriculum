...
;;   r ├────────────────────┼─────────┼───────────────────────────┤
;;   1 │(cons Player Roster)│ false(2)│    (check the <rests>) (4)│
;;     └────────────────────┴─────────┴───────────────────────────┘

(check-expect (all-roster-play? empty empty) true)
(check-expect (all-roster-play? empty R1) false)
(check-expect (all-roster-play? R1 empty) false)
(check-expect (all-roster-play? R1 R1) true)
(check-expect (all-roster-play? R1 R2) false)

;(define (all-roster-play? r1 r2) false)  ;stub

(define (all-roster-play? r1 r2)
  (cond [(and (empty? r1) (empty? r2)) true]    ; 1
        [(or  (empty? r1) (empty? r1)) false]   ; 2
        [else                                   ; 4
         (all-roster-play? (rest r1) (rest r2))]))
...
;; ASSUME: both rosters will have the same length
...
;;     │       empty        │ empty(1)│             x             │
;;   r ├────────────────────┼─────────┼───────────────────────────┤
;;   1 │(cons Player Roster)│    x    │(cons (make-match <firsts>)│
;;     │                    │         │(2)   (list-matches <rest>)│
;;     └────────────────────┴─────────┴───────────────────────────┘

(check-expect (list-matches empty empty) empty)
(check-expect (list-matches (list "Eugenie" "Gabriela")
                            (list "Maria" "Nadia")) (list (make-match "Eugenie" "Maria")
                                                          (make-match "Gabriela" "Nadia")))

;(define (list-matches r1 r2) empty)  ;stub

(define (list-matches r1 r2)
  (cond [(empty? r1) empty]              ; 1
        [else                            ; 2
         (cons (make-match (first r1) (first r2))
               (list-matches (rest r1) (rest r2)))]))