...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname championship-bracket_alt) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
;;    l│     empty        │  true   │               true               │
;;    s├──────────────────┼─────────┼──────────────────────────────────┤
;;    t│(cons String LOT) │  false  │(if (first lst) == (brkt-t2)      │
;;     │                  │         │     check the rest of lst in br1)│
;;     │                  │         │     check for (first lst) in br1)│
;;     └──────────────────┴─────────┴──────────────────────────────────┘
;;
;;  SIMPLIFICATION
;;
...
;;    s├──────────────────┼─────────┼──────────────────────────────────┤
;;    t│(cons String LOT) │  false  │(if (first lst) == (brkt-t2)      │
;;     │                  │   (2)   │     check the rest of lst in br1)│
;;     │                  │         │ (3) check for (first lst) in br1)│
;;     └──────────────────┴─────────┴──────────────────────────────────┘

(check-expect (beaten-teams? false empty) true)
(check-expect (beaten-teams? false (list "Nova")) false)
