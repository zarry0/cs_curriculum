...
;; produces true if los includes s
(check-expect (contains? "UBC" empty) false)
(check-expect (contains? "UBC" (cons "McGill" empty)) false)
(check-expect (contains? "UBC" (cons "UBC" empty)) true)
(check-expect (contains? "UBC" (cons "McGill" (cons "UBC" empty))) true)
...
(check-expect (map2 string-length (list "a" "ab" "abc" "")) (list 1 2 3 0))
...
;; given a predicate q an a list produces a list with all the elements to which the predicate was true 
(check-expect (filter2 positive? empty) empty)
(check-expect (filter2 positive? (list 1 2 4 -5 6)) (list 1 2 4 6))
(check-expect (filter2 negative? (list 1 2 4 -5 6)) (list -5))
(check-expect (filter2 empty? (list 1 2 4 -5 6)) empty)
(check-expect (filter2 false? (list 1 2 4 -5 6)) empty)
