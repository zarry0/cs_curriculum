...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname ellipses) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
(local [(define (oval n) (ellipse n (* 2 n) "solid" "blue"))]
  (build-list 20 oval))
...
; to put the ellipses beside each other in a single image. 
;
...
(local [(define (oval n) (ellipse n (* 2 n) "solid" "blue"))]
  (foldr beside empty-image (build-list 20 oval)))
...
; to put the ellipses beside each other in a single image, but from biggest to smallest.
;


(local [(define (oval n) (ellipse n (* 2 n) "solid" "blue"))]
  (foldl beside empty-image (build-list 20 oval)))