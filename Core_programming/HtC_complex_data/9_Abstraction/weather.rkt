...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname weather) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require racket/file)
...
(check-expect (total-warm-rain (list (make-weather "7/2/88" 21.9 17.7 13.4 0.2 0 0.2))) 0.2)
(check-expect (total-warm-rain (list (make-weather "7/2/88" 21.9 17.7 13.4 0.2 0 0.2)
                                     (make-weather "7/2/88" 21.9 15 13.4 0.5 0 0.8)
                                     (make-weather "7/2/88" 21.9 15.1 13.4 1.2 0 1))) (+ 0.2 1.2))
(check-expect (total-warm-rain WEATHER-DATA) 2545.3)

;(define (total-warm-rain low) 0)  ;stub

(define (total-warm-rain low)
  (local [(define (add-rains w b) (+ (weather-rain w) b))
          (define (avg-tmp>15? w) (> (weather-avg-tmp w) 15))]
    (foldr add-rains 0 (filter avg-tmp>15? low))))

;; Alt. solution
#;
(define (total-warm-rain low)
  (local [(define (avg-tmp>15? w) (> (weather-avg-tmp w) 15))]
    (foldr + 0 (map weather-rain (filter avg-tmp>15? low)))))
