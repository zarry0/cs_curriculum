  ...
  def intersectWithSegmentAsLineResult (seg)
    ...
    if real_close(x1start,x1end)
      if (y1start < y2start)
        ...
        a = [x2start,y2start,x2end,y2end]
        b = [x1start,y1start,x1end,y1end]
      end
      
      if real_close(a[2],b[1])
        Point.new(a[2],a[3])
      elsif a[3] < b[1]
        NoPoints.new()
      elsif a[3] > b[3]
        LineSegment.new(b[0],b[1],b[2],b[3])
      else
        LineSegment.new(b[0],b[1],a[2],a[3])
      end
